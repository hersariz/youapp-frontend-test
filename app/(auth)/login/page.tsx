  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.data && response.data.access_token) {
        console.log("Login successful, access_token received:", response.data.access_token);
        localStorage.setItem('accessToken', response.data.access_token);
        
        // Force a full page reload to ensure localStorage is available on the next page.
        // This is more reliable than router.push for auth flows.
        window.location.href = '/profile';
      } else {
        console.error("Login response did not contain an access_token:", response.data);
      }
    } catch (err: any) {
